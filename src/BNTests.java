...
public class BNTests {
    ...
    public BNTests(BNPoint prototype, BNField12 prototype12){
    	this.prototype= prototype;
    ...
    public static void benchmarks(int BM, int fieldBits) {
        ...
        */
        ...
        for (int i = fieldBits; i <= fieldBits; i++) {
            ...
            for (int t = 0; t < BM; t++) {
                f = f.exp(k);
                PP = P.multiply(k); PP = P.multiply(k); Q = Q.simultaneous(k, k, Q);
            }
            elapsed += System.currentTimeMillis();
            System.out.println("Elapsed time: " + (float)elapsed/BM + " ms.");
            ...
            for (int t = 0; t < BM; t++) {
                f = f.exp(k);
                PP = P.multiply(k); Q = Q.multiply(k);
            }
            elapsed += System.currentTimeMillis();
            System.out.println("Elapsed time: " + (float)elapsed/BM + " ms.");
    ...
    public static void main_test1(String[] args) throws IOException, NoSuchAlgorithmException {
        byte[] randSeed = new byte[20];
        (new Random()).nextBytes(randSeed);
        //*
        for (int i = 0; i < randSeed.length; i++) {
        ...
        for (int i = 0; i < BNParams.validBitsRange.length; i++) {
            ...
            BNCurve E = new BNCurve(bn); System.out.println("E: " + E);
            E.G.getSerializedTable();
//            BNCurve2 Et = new BNCurve2(E); System.out.println("Et: "+Et);
//            BNTests T = new BNTests(E.G, Et.Gt, E.bn.Fp12_0);
            BNTests T= new BNTests(E.G, E.bn.Fp12_0);
...
//            BNPoint2 Q = Et.Gt;
            ...
            //*/
            /*
            System.out.println("Testing optimal pairing");
            g = pair.opt(Et.Gt, E.G);
            //System.out.println("g = " + g);
            //System.out.println("g^n = " + g.exp(bn.n));
            if (g.isZero()) {
                throw new RuntimeException("degeneracy error!");
            ...
            a = pair.opt(Et.Gt.twice(1), E.G);
            b = pair.opt(Et.Gt, E.G.twice(1));
            c = g.square();
            System.out.println("bilinear? " + (a.equals(b) && b.equals(c)));
            if (!(a.equals(b) && b.equals(c)) || a.isOne()) {
                System.out.println(">>>> a = " + a);
            ...
            //*/
            /*
            System.out.println("Benchmarking simultaneous:");
            elapsed = -System.currentTimeMillis();
            for (int t = 0; t < BM; t++) {
                PP = E.G.simultaneous(k, k, P);
            }
            ...
            long fpmcount = BNPairing.getfpm();
            ...
            System.out.println("equiv Fp mul = " + (fpmcount + addcount/17 + 27*modcount/10));
            //*/
    ...
    public static void main_test0(String[] args) throws IOException, NoSuchAlgorithmException {
        benchmarks(100, 158);
    }
}
