...
public class BNPoint {
    ...
    BNPoint(BNCurve E, int xTrit, BigInteger y) {
        ...
        if (y.signum() == 0) {
            ...
            if (x.mod(E.bn._3).intValue() != xTrit) {
    ...
    public BNPoint normalize() {
        if (z.signum() == 0 || z.compareTo(E.bn._1) == 0) {
        ...
        return new BNPoint(E, x.multiply(zinv2).mod(p), y.multiply(zinv).multiply(zinv2).mod(p), E.bn._1);
    }

    public boolean isNormal() {
        return (z.signum() == 0 || z.compareTo(E.bn._1) == 0);
    }
    ...
    public BNPoint add(BNPoint Q) {
        ...
        BigInteger p = E.bn.p;
        BigInteger X1 = x, Y1 = y, Z1 = z, X2 = Q.x, Y2 = Q.y, Z2 = Q.z, Z1Z1 = E.bn._1, Z2Z2 = E.bn._1, U1 = x, U2 = Q.x, S1 = y, S2 = Q.y, H, I, J, R, V, X3, Y3, Z3;
        boolean Z1is1 = (Z1.compareTo(E.bn._1) == 0);
        boolean Z2is1 = (Z2.compareTo(E.bn._1) == 0);
        if (!Z1is1) {
        ...
        BigInteger p = E.bn.p; // shorthand
        BigInteger t1, t2, t3, t4, t5, t6, t7, M;
        t1 = x;
        t2 = y;
        t3 = z;
        t4 = Q.x;
        ...
        // R = s0 - s1
        M = t2.subtract(t5).mod(p);
        if (t7.signum() == 0) {
            return (M.signum() == 0) ? Q.twice(1) : E.infinity;
        }
        // T = u0 + u1
        t1 = t1.add(t4);//.mod(p);
        // M = s0 + s1
        ...
        t1 = M.multiply(M).subtract(t6).mod(p);
        // 2.y2 = (T.W^2 - 2.x2).R - M.W^2.W
        t2 = t6.subtract(t1.shiftLeft(1)).multiply(M).subtract(t2.multiply(t5).multiply(t7)).mod(p);
    ...
    public BNPoint twice(int n) {
        ...
        while (n-- > 0) {
            ...
            M = A.multiply(E.bn._3).mod(p); // M = 3*A
    ...
    public BNPoint multiply(BigInteger k) {
        if (pp16P == null) {
            ...
            //*
            BigInteger r = bn.u.shiftLeft(1).add(E.bn._1); // 2*u + 1
            BigInteger t = bn.u.multiply(E.bn._3).add(E.bn._1).multiply(bn.u.shiftLeft(1)); // (3*u + 1)*2*u = 6*u^2 + 2*u
            //*/
            ...
            if (kr.mod(bn.n).compareTo(halfn) <= 0) {
                ...
                kr = kr.divide(bn.n).add(E.bn._1);
            }
            BigInteger kt = k.multiply(t);
            if (kt.mod(bn.n).compareTo(halfn) <= 0) {
                kt = kt.divide(bn.n);
            } else {
                kt = kt.divide(bn.n).add(E.bn._1);
            }
