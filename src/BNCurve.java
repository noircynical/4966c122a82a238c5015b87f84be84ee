...
public class BNCurve {
    ...
    public BNCurve(BNParams bn) {
        this.bn = bn;
        b = (bn.b == 5) ? _5 : (bn.b == 3) ? _3 : _2; // standard curve
        infinity = new BNPoint(this); // caveat: must be set *after* p but *before* G!
        G = (bn.b == 5) ? new BNPoint(this, _1.negate(), _2) : (bn.b == 3) ? new BNPoint(this, _1, _2) : new BNPoint(this, _1.negate(), _1); // standard curve
        /*
        ...
        pp16Gserial = new byte[(bn.n.bitLength() + 3)/4][16][];
        long elapsed = -System.currentTimeMillis();
        for (int qqq = 0; qqq < 1; qqq++) {
            BNPoint[] pp16Gi = pp16G[0];
            pp16Gi[0] = infinity;
            pp16Gi[1] = G;
            for (int i = 1, j = 2; i <= 7; i++, j += 2) {
                pp16Gi[j  ] = pp16Gi[i].twice(1).normalize();
                pp16Gi[j+1] = pp16Gi[j].add(G).normalize();
            }
            for (int i = 1; i < pp16G.length; i++) {
                BNPoint[] pp16Gh = pp16Gi;
                pp16Gi = pp16G[i];
                pp16Gi[0] = pp16Gh[0];
                for (int j = 1; j < 16; j++) {
                    pp16Gi[j] = pp16Gh[j].twice(4).normalize();
                }
            }
        }
        ...
        /*
        System.out.println("========");
        for (int i = 0; i < pp16G.length; i++) {
            for (int j = 0; j < pp16G[i].length; j++) {
                System.out.println("pp16G[" + i + "][" + j + "] = " + pp16G[i][j]);
            }
        }
        System.exit(0);
        //*/
        ...
        for (int i = 0; i < pp16G.length; i++) {
            for (int j = 0; j < pp16G[i].length; j++) {
                pp16Gserial[i][j] = pp16G[i][j].toByteArray(BNPoint.EXPANDED);
            }
        }
        elapsed = -System.currentTimeMillis();
        for (int qqq = 0; qqq < 1; qqq++) {
            for (int i = 0; i < pp16G.length; i++) {
                for (int j = 0; j < pp16G[i].length; j++) {
                    pp16G[i][j] = new BNPoint(this, pp16Gserial[i][j]);
                }
            }
        }
        elapsed += System.currentTimeMillis();
        System.out.println("pp16G deserialization = " + elapsed);
        //*/
    }
