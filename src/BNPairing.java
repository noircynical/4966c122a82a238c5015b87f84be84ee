...
//    BNField12 gl(BNPoint V, BNPoint P) {
//        BigInteger n, d;
//        BigInteger p = bn.p;
...
//    }
//    
//    public BNField12 tate(BNPoint P) {
//        assert (E.contains(P));
//        BNField12 f = Fp12_1;
//        P = P.normalize();
//        if (!P.isZero()) {
...
//            	f= f.square().multiply(gl(V,V));
//                V = V.twice(1);
//                if (bn.n.testBit(i)) {
//                    f= f.square().multiply(gl(V,P));
//                    V = V.add(P);
//                }
//            }
//            f = f.finExp();
...
//    }
    ...
    /*
    BNField12 gl(BNPoint2 T, BNPoint2 Q, BNPoint P) {
        BNField2 n, d;
        if (T.isZero() || P.isZero() || Q.isZero()) {
            return Fp12_1;
        }
        assert (!T.opposite(Q));
        ...
        if (T.equals(Q)) {
            ...
            // x = X/Z^2 => 3x^2 = 3X^2/Z^4 =>
            // => lambda = 3x^2/2y = 3X^2/(2Y*Z)
            n = T.x.square().multiply(BigInteger.valueOf(3L));
    ...
    /*
    // Naive implementation of the ate pairing
    public BNField12 ate(BNPoint2 Q, BNPoint P) {
        ...
        if (!P.isZero() && !Q.isZero()) {
            ...
            for (int i = ord.bitLength() - 2; i >= 0; i--) {
                f = f.square().multiply(gl(T, T, P));
                T = T.twice(1);
                if (ord.testBit(i)) {
                    f = f.multiply(gl(T, Q, P));
                    T = T.add(Q);
                }
            }
            f = f.finExp();
    ...
    public BNField12 ate(BNPoint2 Q, BNPoint P) {
        ...
        Q = Q.normalize();
        if (!P.isZero() && !Q.isZero()) {
            BigInteger ord = bn.t.subtract(E.bn._1);
    ...
    /*
    // naive implementation of the optimal pairing
    public BNField12 opt(BNPoint2 Q, BNPoint P) {
        ...
        if (!P.isZero() && !Q.isZero()) {
            ...
            f = f.multiply(gl(Q2, Q3, P)).multiply(gl(Q4, Q1, P));
            f = f.finExp();
        }
        return f;
    }
