...
public class BNField2 {
    ...
    public boolean isOne() {
        return re.compareTo(bn._1) == 0 && im.signum() == 0;
    }
    ...
    public BNField2 negate() {
        return new BNField2(bn, (re.signum() != 0) ? bn.p.subtract(re) : re, (im.signum() != 0) ? bn.p.subtract(im) : im, false);
    }
    ...
    public BNField2 halve() {
        return new BNField2(bn,
            (re.testBit(0) ? re.add(bn.p) : re).shiftRight(1),
            (im.testBit(0) ? im.add(bn.p) : im).shiftRight(1),
            false);
    }
    ...
    public BNField2 multiply(BigInteger v) {
        /*
        if (docount && v.bitLength() > SMALLSIZE) {
            fpmcount += 2;
        }
        //*/
    ...
    public BNField2 square() {
        ...
        if (re.signum() == 0) {
            ...
            //*/
            return new BNField2(bn,
                im.multiply(im).negate(), bn._0, true);
        }
        //*
        if (docount) {
            addcount += 2;
    ...
    public BNField2 cube() {
        ...
        return new BNField2(bn,
            re.multiply(re2.subtract(im2.add(im2).add(im2))),
            im.multiply(re2.add(re2).add(re2).subtract(im2)),
            true);
    }
    ...
    public BNField2 sqrt() {
        ...
        r2 = r2.multiplyI();
        //BNField2 sqrtI = new BNField2(bn, bn.invSqrtMinus2, bn.p.subtract(bn.invSqrtMinus2), false); // sqrt(i) = (1 - i)/sqrt(-2)
    ...
    public BNField2 cbrt() {
        assert (bn.p.mod(bn._9).intValue() == 4);
        if (this.isZero()) {
            return this;
        }
