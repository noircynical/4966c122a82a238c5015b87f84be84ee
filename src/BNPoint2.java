...
public class BNPoint2 {
    ...
    BNPoint2(BNCurve2 E, int xTrit, BNField2 y) {
        ...
        if (y.isZero()) {
            ...
            if (x.re.mod(E.E.bn._3).intValue() != xTrit) {
    ...
    public BNPoint2 add(BNPoint2 Q) {
        ...
        BNField2 X1 = x, Y1 = y, Z1 = z, X2 = Q.x, Y2 = Q.y, Z2 = Q.z, Z1Z1 = Fp2_1, Z2Z2 = Fp2_1, U1 = x, U2 = Q.x, S1 = y, S2 = Q.y, H, I, J, R, V, X3, Y3, Z3;
        ...
        BNField2 t1, t2, t3, t4, t5, t6, t7, M;
        t1 = x;
        t2 = y;
        t3 = z;
        t4 = Q.x;
        ...
        M = t2.subtract(t5);
        if (t7.isZero()) {
            return M.isZero() ? Q.twice(1) : E.infinity;
        }
        // T = u0 + u1
        t1 = t1.add(t4);
        // M = s0 + s1
        ...
        t1 = M.square().subtract(t6);
        // 2.y2 = (T.W^2 - 2.x2).R - M.W^2.W
        t2 = t6.subtract(t1.twice(1)).multiply(M).subtract(t2.multiply(t5).multiply(t7)).halve();
        // lambda = (Q.y - T.y)/(Q.x - T.x) // Q.Z = 1
        // = (y1 - y0/z0^3) / (x1 - x0/z0^2)
        // = (y1*z0^3 - y0) / (x1*z0^3 - x0*z0)
        // = (y0 - y1*z0^3) / z0*(x0 - x1*z0^2)
    ...
    public BNPoint2 multiply(BigInteger k) {
        ...
        u[0] = u[0].add(k);
        BNPoint2 Q = P.frobex(1);
        BNPoint2 R = P.frobex(2);
        BNPoint2 S = P.frobex(3);
    ...
    public BNPoint2 frobex(int k) {
        ...
        switch (k) {
        case 1:
            return (bn.b == 3 || bn.b == 5) ?
                new BNPoint2(E,
                    x.multiplyI().conjugate().multiply(bn.zeta),
                    y.multiplyV().conjugate().multiply(bn.zeta0sigma),
                    z) :
                new BNPoint2(E,
                    x.conjugate().multiplyI().multiply(bn.zeta),
                    y.conjugate().multiplyV().multiply(bn.zeta1sigma),
                    z);
        case 2:
        ...
        case 3:
            return (bn.b == 3 || bn.b == 5) ?
                new BNPoint2(E,
                    x.multiplyI().conjugate(),
                    y.multiplyV().conjugate().multiply(bn.zeta0sigma).negate(),
                    z) :
                new BNPoint2(E,
                    x.conjugate().multiplyI(),
                    y.conjugate().multiplyV().multiply(bn.zeta1sigma).negate(),
                    z);
        default:
            return null;
        }
